...
export class StatisticsPage extends Component {
        ...
        return (
            <div>
                ...
                <div className="mt-3" align="center">
                    ...
                    <div>
                        {this.lineChart ? (this.lineChart) : (<div className="mt-5">Ingen saker registrert i gjeldende kommune dette årstallet.</div>)}
                    </div>
                </div>
            </div>
        );
    ...
    updateChart() {
        this.stats = [{x: 'Januar', y: 0}, {x: 'Februar', y: 0}, {x: 'Mars', y: 0}, {x: 'April', y: 0},
            {x: 'Mai', y: 0}, {x: 'Juni', y: 0}, {x: 'Juli', y: 0}, {x: 'August', y: 0},
            {x: 'September', y: 0}, {x: 'Oktober', y: 0}, {x: 'November', y: 0}, {x: 'Desember', y: 0}];

        issueService.getNumberOfIssues(this.munId, this.year)
            .then(stats => {
                if (stats.length > 0) {
                    stats.map(stat => {
